...
output = render('HAZARD',colors=['green','white'], align='center')
print(output)
print(GG+'-'*1+W+'{ '*1+GG+'@H_7_Z_W'*1+W+' }'*1+GG+'-'*1+W+'{'*1+Z1+' 𝐇𝐀𝐙𝐀𝐑𝐃🇸🇩'*1+W+' }'*1+F'-'*1+GG+W+'{'*1+GG+'@H_7_Z_W'*1+W+'}'*1)
print('')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s 𝐇𝐀𝐙𝐀𝐑𝐃🇸🇩 %s/%s 🔥HZ ⌠𝗢𝗞𖤍⌡ %s 🔥HZ ⌠𝗖𝗣𖤍⌡ %s 𖤤 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
@H_7_Z_W '''
					infoakun1 = nel(infoakun, style='green')
					print (infoakun1) 
